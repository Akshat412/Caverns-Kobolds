...
{
	int choice, landchance, chance, temp, damage_dealt, stressfac, stress;
	temp = e.HP;
		...
		case 1:
		{
			...
			switch(choice)
			{
				case 1:
				{
					...
					if((p.w.weight/2 - p.wh + e.sdodge) > 0)chance = p.w.weight/2 - p.wh + e.sdodge;
				...
				case 2:
				{
					...
					if((p.w.weight/5 + e.sdodge - p.wh + stressfac * p.smDEX) > 0) chance = p.w.weight + e.sdodge - p.wh/2 + stressfac * p.smDEX; else chance = 1;
		...
		case 2:
		{
			...
			switch(choice)
			{
				case 1:
				{
					...
					chance = p.w.weight/2 - p.wh/10 + e.sdodge + stressfac * p.smDEX; 
